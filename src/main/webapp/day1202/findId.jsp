...
#container{ height: 380px; position:relative;	
	background: #FFFFFF url(http://localhost/jsp_prj/day1202/image/id_background.png) no-repeat; }	
...
<div id="wrap">
	...
	<form name="popupFrm" action="findId.jsp" method="get">
		<input type="text" name="id" value="${ param.id }" style="width:200px"/>
	...
	<div id="searchResult">
	<%
		WebMemberService wmSerivce=WebMemberService.getInstance();
		boolean flagId=wmSerivce.searchId(request.getParameter("id"));
	...
	입력하신 <strong>${ param.id }</strong>는 <span class="${ resultCss }"><c:out value="${ resultMsg }"/></span>
	<c:if test="${ flagId }">
